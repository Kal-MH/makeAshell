...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/12/05 04:34:29 by napark            #+#    #+#             */
/*   Updated: 2021/12/24 00:56:45 by mkal             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	get_next_line(int fd, char **line)
{
	...
	if (fd < 0 || line == 0 || BUFFER_SIZE <= 0
		|| !ft_calloc(sizeof(char), BUFFER_SIZE + 1, (void *)&buf))
		return (GNL_ERROR);
	while (1)
	{
		gnl_read = read(fd, buf, BUFFER_SIZE);
		if ((get_index(back[fd]) != -1) || gnl_read <= 0)
			break ;
		buf[gnl_read] = '\0';
		tmp = back[fd];
